...
export default function InterviewPrepApp() {
  ...
  const finishInterview=(responses,fmt2)=>{
    ...
    else if(fmt2==='mc'){ setPage('results'); } // MC always free, no gate
    else { setPendingInterview(iv); setPage('results-gate'); } // written/mock: email gate
  };
  ...
  const verifyResultsGate = async () => {
    ...
    try {
      ...
      if (error) { setAuthError('Invalid code. Please try again.'); }
        ...
        // Verified — save interview and show results
        if (pendingInterview) { saveInterview(pendingInterview); }
            ...
            <div style={{flex:1,overflowY:'auto'}} className="mobile-content">
              ...
              {page==='roles' && (()=>{
                ...
                const INTERVIEW_GUIDES = {
                  ...
                  'Forward Deployed Engineer': {
                    process: [
                      { stage: 'Recruiter screen', duration: '30 min', what: 'Why Palantir or the specific company, what defines your job search, and which role (SWE vs FDE) fits you better. Read the Q4 shareholder letter before this call. Use the vocabulary: ontology, transforms, AIP, Phase 4 adoption. Candidates who speak only in generalities are typically screened out at this stage.' },
                      ...
                      { stage: 'System design', duration: '60 min', what: 'Distributed systems design at the scale Palantir operates. Correctness and fault tolerance are first-class constraints. Candidates who treat these as afterthoughts rarely advance.' },
                  ...
                  'Forward Deployed Product Manager': {
                    process: [
                      ...
                      { stage: 'Technical fluency check', duration: '45 min', what: 'Not a coding interview. Expect to discuss: how would you write requirements for an LLM feature, how would you define success metrics for a RAG deployment, what questions would you ask an engineering team to assess timeline feasibility. The goal is to confirm you can hold a credible technical conversation with an engineering team.' },
                ...
                const ROLE_ARTICLES = [
                  {
                    ...
                    what: "An AI Solutions Architect is a customer-facing technical role that helps enterprises design and adopt AI systems. The work centers on discovery and architecture: understanding a customer's data landscape, infrastructure, and business goals, then designing the system that connects what the customer has today to what a production AI deployment requires. Anthropic describes its Applied AI SA role as a trusted technical advisor to large enterprises. At AWS and Google Cloud, the role follows the established cloud SA model, applied to AI workloads.",
                    notJust: "The role is distinct from both engineering and consulting. An AI SA writes little production code, but must defend technical decisions in front of engineers who will challenge them. The job requires enough depth to hold credibility with an engineering team and enough breadth to communicate with a CTO or VP of Product. Typical output is architecture documents, proposals, and recommendations rather than code. Most working hours go to discovery sessions, architecture reviews, and stakeholder presentations.",
                    dayToDay: [
                    ...
                    skills: [
                      ...
                      { name: 'Evaluation and Measurement', desc: 'Defining and implementing evaluation frameworks: retrieval recall@k, answer faithfulness, hallucination rate, latency p95. Evaluation design is frequently the deciding factor in whether a deployment succeeds.' },
                  ...
                  },
                  {
                    ...
                    what: "A Forward Deployed Engineer is a software engineer embedded with a specific customer to build and deploy AI systems in that customer's environment. Palantir originated the role around 2010; for several years the company employed more FDEs than conventional software engineers. The model has since been adopted widely — OpenAI, Anthropic, Ramp, Databricks, Scale AI, Stripe, and Salesforce all operate FDE teams, and job postings for the role grew more than 800% between 2024 and 2025. The reason is consistent across companies: enterprise AI deployments stall between trial and production, and embedding engineers directly with the customer is the most reliable way to close that gap.",
                    notJust: "Palantir's job description compares FDE responsibilities to those of a startup CTO: small teams, end-to-end ownership of high-stakes projects. The accountability is to a specific customer's outcome rather than a product metric, and all code runs against real data in production systems. The working environments are unusual for software engineering — former Palantir FDE Anjor Kanekar, who spent seven years in the role, has described engagements on the Airbus final assembly line and in airgapped government facilities.",
                    dayToDay: [
                  ...
                  },
                  {
                    ...
                    notJust: "The role differs substantially from traditional product management. There are no A/B tests or usage dashboards; the work is closer to management consulting — diagnosis, scoping, relationship management, and expectation-setting. An FDPM routinely navigates three gaps at once: between the customer's stated problem and their actual problem, between the formal org chart and how decisions actually get made, and between what engineering can deliver in the timeline and what the customer expects.",
                    dayToDay: [
                  ...
                  },
                  {
                    ...
                    notJust: "TPM is frequently mistaken for project management with a technical title, but the roles differ in kind. A project manager tracks work that is already defined; a TPM identifies the dependency that will derail a launch six weeks out and drives the cross-team resolution now, while it is still inexpensive to fix. The technical qualifier is substantive: a TPM who cannot read a design document, assess engineering complexity, or credibly push back on a timeline will not hold standing with the teams they coordinate.",
                    dayToDay: [
                    ...
                    skills: [
                      ...
                      { name: 'Dependency and Critical Path Analysis', desc: 'Modeling complex programs to find the true critical path and identifying the constraint that, if removed, unblocks the most downstream work. The actual critical path frequently differs from what individual teams assume it is.' },
                  ...
                  {emailGateStep==='email' ? (
                    <div className="card" style={{padding:'24px'}}>
                      <div style={{marginBottom:14}}>
                        ...
                        <input type="email" placeholder="you@company.com" autoFocus
                          value={waitlistEmail} onChange={e=>setWaitlistEmail(e.target.value)}
                          onKeyDown={e=>e.key==='Enter'&&submitResultsGate()}
                          style={{width:'100%',padding:'11px 14px',border:'1px solid #E5E7EB',borderRadius:8,fontSize:14,color:'#111827',background:'#F9FAFB'}}/>
                      </div>
                      ...
                      <button onClick={()=>handleOAuth('google')} style={{width:'100%',padding:'11px',border:'1px solid #E5E7EB',borderRadius:8,background:'#fff',cursor:'pointer',fontSize:14,fontWeight:500,color:'#374151',display:'flex',alignItems:'center',justifyContent:'center',gap:10,marginBottom:8}}>
                        <svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                        Continue with Google
                      </button>
                      <button onClick={()=>handleOAuth('github')} style={{width:'100%',padding:'11px',border:'1px solid #E5E7EB',borderRadius:8,background:'#fff',cursor:'pointer',fontSize:14,fontWeight:500,color:'#374151',display:'flex',alignItems:'center',justifyContent:'center',gap:10}}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="#24292E"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/></svg>
                        Continue with GitHub
                      </button>
                    </div>
                    ...
                    <div className="card" style={{padding:'24px'}}>
                      <p style={{fontSize:14,color:'#6B7280',marginBottom:16,textAlign:'center'}}>We sent a 6-digit code to <strong>{waitlistEmail}</strong></p>
                      <div style={{marginBottom:14}}>
                        <input type="text" placeholder="123456" autoFocus maxLength={6}
                          value={emailGateOtp} onChange={e=>setEmailGateOtp(e.target.value.replace(/\D/g,''))}
                          onKeyDown={e=>e.key==='Enter'&&verifyResultsGate()}
                          style={{width:'100%',padding:'12px 14px',border:'1px solid #E5E7EB',borderRadius:8,fontSize:24,color:'#111827',background:'#F9FAFB',textAlign:'center',letterSpacing:'0.3em',fontWeight:600}}/>
                      </div>
                      <button className="bp" onClick={verifyResultsGate} disabled={emailGateOtp.length<6||waitlistWorking}
                        style={{width:'100%',padding:'12px',fontSize:15,display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:10}}>
                        {waitlistWorking?<><span className="spinner"/>Verifying…</>:'See my results →'}
                      </button>
                      <button onClick={()=>{setEmailGateStep('email');setEmailGateOtp('');setAuthError('');}}
                        style={{background:'none',border:'none',cursor:'pointer',width:'100%',fontSize:13,color:'#9CA3AF',textAlign:'center'}}>
                        Use a different email
